...
class ShuffleNetV1(nn.Module):
    def __init__(self, input_size=224, n_class=1000, model_size='2.0x', group=None):
        ...
        if group == 3:
            ...
            if model_size == '0.5x':
                self.stage_out_channels = [-1, 16, 136, 272, 544]
            elif model_size == '1.0x':
                self.stage_out_channels = [-1, 24, 272, 544, 1088]
            elif model_size == '1.5x':
                self.stage_out_channels = [-1, 24, 408, 816, 1632]
            elif model_size == '2.0x':
                self.stage_out_channels = [-1, 48, 544, 1088, 2176]
            else:
                raise NotImplementedError
        ...
        for idxstage in range(len(self.stage_repeats)):
            ...
            for i in range(numrepeat):
                ...
                self.features.append(ShuffleV1Block(input_channel, output_channel,
                                            ...
                                            mid_channels=output_channel // 4, ksize=3, stride=stride, iscs=iscs))
                iscs = False
